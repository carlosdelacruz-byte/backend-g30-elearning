...
class Tragamonedas:
    def _init_(self, nombre, creditos):
        self.nombre = nombre
...
class TragamonedasCriolla(Tragamonedas):
    def _init_(self, nombre, creditos):
        super()._init_(nombre, creditos)
    ...
    def jugar(self, apuesta):
        ...
        es_ceviche = "🐟" in resultado and "🍋" in resultado and "🌶️" in resultado

        if resultado[0] == resultado[1] == resultado[2]:
            premio = apuesta * 12
            print(f"🏆 ¡TRIPLE CRIOLLO! Ganaste S/ {premio}")
            self._creditos += premio
            self._total_ganado += premio
        elif es_ceviche:
            premio = apuesta * 8
            print(f"🐟🍋🌶️  ¡CEVICHE COMPLETO! Ganaste S/ {premio}")
            self._creditos += premio
            self._total_ganado += premio
        elif resultado[0] == resultado[1] or resultado[1] == resultado[2] or resultado[0] == resultado[2]:
            premio = apuesta * 2
...
maquinas = [
    ...
    TragamonedasCriolla("La Criolla", 150),
]
